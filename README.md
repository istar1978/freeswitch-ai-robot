...
- **MySQL存储**：使用MySQL数据库存储用户、配置和通话记录
- **Docker部署**：支持容器化部署和编排
...
- **Web管理界面**：基于Web的图形化管理界面，支持FreeSWITCH配置、场景管理、外呼控制
- **多FreeSWITCH支持**：可配置多个FreeSWITCH服务器实例及其场景映射
- **实时监控**：实时显示活跃通话和对话记录，支持WebSocket实时更新
- **用户认证**：内置用户认证和权限管理系统，从.env文件读取默认管理员密码
- **数据库存储**：用户账号、系统配置、通话记录（含多轮交互转义文字）存储在MySQL数据库
...
- **storage/**: 数据存储（Redis客户端、MySQL客户端和数据模型）
- **utils/**: 工具类（日志、助手函数）
...
- **WebUI管理界面**: `http://localhost:8081` - Web管理界面
...
项目提供了基于Web的图形化管理界面，用于方便地管理系统配置和监控运行状态。

### 访问地址

启动服务后，通过浏览器访问：`http://localhost:8081`
